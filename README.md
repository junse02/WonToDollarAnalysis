...
- **상위 종목 시세·차트** — 국내(`/stocks`, 원화)는 대형주와 원자력·방산·2차전지 등 테마 대표주 16개, 미국(`/stocks/us`, 달러)은 대형주와 양자컴퓨팅·AI반도체 등 테마 대표주 14개 종목의 일별 시세(Yahoo Finance, 무인증)와 30/90일 종가 차트, 전일 대비 변동액·변동률·상승/하락 방향을 보여줍니다. 상단 버튼으로 두 시장을 전환합니다.
- **섹터·테마별 강세·관심도 요약** — 페이지 상단에 업종/테마별 **평균 등락률(강세)**과 **뉴스 언급량(관심도, 네이버 검색 건수)**을 집계해, "지금 어느 섹터·테마가 강하고 주목받는지"를 한 줄 요약 배지와 강세순 타일로 보여줍니다. 국내는 반도체·2차전지·방산·원자력 등, 미국은 양자컴퓨팅·AI반도체·빅테크 등 시장 테마 단위로 묶어 표시합니다.
...
| `GET` | `/api/stocks` | 국내 종목(대형주 + 원자력·방산·2차전지 등 테마 대표주) 시세 + 뉴스 감성 (JSON) |
| `GET` | `/api/stocks/us` | 미국 종목(대형주 + 양자컴퓨팅·AI반도체 등 테마 대표주) 시세 + 뉴스 감성 (JSON) |
